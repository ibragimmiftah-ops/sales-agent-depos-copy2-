...
**NovaFlow AI Sales Agent** — автономный AI-агент первой линии продаж. Он не «отвечает на сообщения», а ведёт процесс: анализирует состояние лида, выбирает следующее действие, вызывает внешние инструменты (CRM, Calendar, RAG), изменяет состояние внешних систем и двигает пользователя к бизнес-цели — qualified lead + booked meeting.
